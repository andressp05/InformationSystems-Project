...
from datamodel import constants, forms
from datamodel.models import Counter, Game, Move
GameStatus = Game.GameStatus
from datamodel.tests import BaseModelTest
import os
import django

os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'foxHound.settings')
django.setup()


TEST_USERNAME_1 = "testUserMouseCatBaseTest_1"
TEST_PASSWORD_1 = "pass1"
TEST_USERNAME_2 = "testUserMouseCatBaseTest_2"
TEST_PASSWORD_2 = "pass2"
...
# --- DO NOT MODIFY THE TEST BELOW THIS LINE ----
...
class LogInOutServiceTests(ServiceBaseTest):
    ...
    def test1(self):
        ...
        available. Do not try to log.
        if we do not login "_auth_user_id" should not
        be available
        """
    ...
    def test2(self):
        """ Connect to login page, fill the form and
        send request. First user is logged. Check
        that django user validation system has been used.
        _auth_user_id should be available

        Star a second client (other client with other session)
        try to get _auth_user_id. Should not be available
         """
        ...
        self.client1.post(reverse(LOGIN_SERVICE),
                          ...
                          follow=True)
        self.assertEqual(
            Decimal(self.client1.session.get(USER_SESSION_ID)),
            self.user1.id)
    ...
    def test3(self):
        """ROB This one in too complex"""
        self.validate_anonymous_required(self.client1, LOGIN_SERVICE)

    def test4(self):
        """Test login followed by loading home"""
        sessions = [
    ...
    def test7(self):
        """ Check that logout works cleans session"""
...
class SignupServiceTests(ServiceBaseTest):
    ...
    def test0(self):
        """Check that user creation form does not
        return errors"""
        User.objects.filter(id=self.user1.id).delete()
        self.assertTrue(forms.SignupForm(self.paramsUser1).is_valid())

    def test1(self):
        """try to sign in after login in.
    ...
    def test4(self):
        """ROB: I do not understand what is check here"""
        User.objects.filter(id=self.user1.id).delete()
        pass_dummy = self.paramsUser1["username"] + "XXX"
        User.objects.create(username=self.paramsUser1["username"],
                            password=pass_dummy)
        n_user = User.objects.count()
        print("n_user", n_user)
        user = User.objects.get(username=self.paramsUser1["username"])
        self.assertEqual(user.username, self.paramsUser1["username"])
        print("kk", user.password, pass_dummy)
...
class CounterServiceTests(ServiceBaseTest):
    ...
    def test1(self):
        """ Call several time to counter from 2 different
        clients"""
        for i in range(1, 4):
            response = self.client1.get(reverse(COUNTER_SERVICE),
                                        follow=True)
            self.is_counter_session(response, i)
        for i in range(1, 3):
            response = self.client2.get(reverse(COUNTER_SERVICE),
                                        follow=True)
            self.is_counter_session(response, i)
        for i in range(4, 6):
            response = self.client1.get(reverse(COUNTER_SERVICE),
    ...
    def test2(self):
        """call counter_service from two different
        clients and check that counters are OK"""
        c = Counter()
        n_calls = c.get_value()
        for _ in range(2):
...
class LogInOutCounterServiceTests(ServiceBaseTest):
    ...
    def test1(self):
        """ Test counter when session
        changes due to log-in log-out"""
        c = Counter()
        n_calls = c.get_value()
...
class CreateGameServiceTests(GameRequiredBaseServiceTests):
    ...
    def test1(self):
        "Try to create a game before login in"
        self.validate_login_required(self.client1, CREATE_GAME_SERVICE)

    def test2(self):
        """Create game, retrieve last
        created game and check parameters"""
...
class JoinGameServiceTests(BckGamesServiceTests):
    ...
    def test1(self):
        """Try to joiun a game before login"""
...
class CleanOrphanGamesServiceTests(BckGamesServiceTests):
    def setUp(self):
        super().setUp()
    ...
    def test1(self):
        self.validate_login_required(self.client1, CLEAN_SERVICE)
    ...
    def test2(self):
        ...
        self.assertEqual(Game.objects.filter(mouse_user__isnull=True).count(), n_orphans)
        self.assertEqual(Game.objects.filter(status=GameStatus.CREATED).count(), n_orphans)
        self.loginTestUser(self.client1, self.user1)
        response = self.client1.get(reverse(CLEAN_SERVICE), follow=True)
        self.is_clean_db(response, n_orphans)
        self.assertEqual(Game.objects.filter(mouse_user__isnull=True).count(), 0)
        self.assertEqual(Game.objects.filter(status=GameStatus.CREATED).count(), 0)
